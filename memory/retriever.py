...
def recency_score(updated_at: str) -> float:
    ...
    age_days: float = max(
        0.0, (datetime.now(timezone.utc) - updated.astimezone(timezone.utc)).total_seconds() / 86400.0
    )
    return 1.0 / (1.0 + age_days / 30.0)


def category_relevance(category: str, query: str) -> float:
    return 1.0 if category.casefold() in query.casefold() else 0.0
...
def rank_memory(record: MemoryRecord, query: str) -> RankedMemory:
    ...
    score: float = (
        ...
        + 0.15 * recency_score(record.updated_at)
        + 0.10 * category_relevance(record.category, query)
        + 0.05 * access
    )
