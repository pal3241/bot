...
from memory.extractor import ParsedMemoryResponse, parse_explicit_memory_command, parse_memory_response, structured_response_instruction
...
from config import AI_SETTINGS_FILE, LLM_PROVIDER, LLM_MAX_TOKENS, LLM_REQUEST_TIMEOUT_SECONDS, LLM_RETRY_COUNT, LLM_RETRY_DELAY_SECONDS, NVIDIA_NIM_BASE_URL, NVIDIA_NIM_MODEL, OPENROUTER_MODEL, SENA_CHAT_TIMEOUT_SECONDS, SENA_HISTORY_MAX_MESSAGES, SENA_PERSONALITY_FILE
...
class AssistantManager:
    def __init__(self, personality: PersonalityManager, sessions: SessionManager, llm: LLMManager, settings: AISettings, owner_resolver: OwnerResolver, memory: MemoryManager) -> None:
        ...
        self._history_context_max_chars = _positive_int_env("SENA_HISTORY_CONTEXT_MAX_CHARS", 4000 if device.is_android else 8000)
        self._llm_slots = asyncio.Semaphore(self._llm_concurrency)
        print(f"[SENA PERF] llm_concurrency={self._llm_concurrency} history_context_max_chars={self._history_context_max_chars}")
    ...
    async def close(self) -> None:
        ...
        try: await self.memory.close()
        except aiosqlite.Error as error: print(f"[SENA MEMORY] close failed type={type(error).__name__} detail={error}")
        self.sessions.clear()
...
def build_assistant_manager() -> AssistantManager:
    settings = load_settings(AI_SETTINGS_FILE, AISettings(provider_name=os.getenv("LLM_PROVIDER", LLM_PROVIDER).strip().lower(), openrouter_model=os.getenv("OPENROUTER_MODEL", OPENROUTER_MODEL).strip(), nvidia_nim_model=os.getenv("NVIDIA_NIM_MODEL", NVIDIA_NIM_MODEL).strip(), nvidia_nim_base_url=os.getenv("NVIDIA_NIM_BASE_URL", NVIDIA_NIM_BASE_URL).strip(), max_tokens=LLM_MAX_TOKENS, request_timeout_seconds=LLM_REQUEST_TIMEOUT_SECONDS, retry_count=LLM_RETRY_COUNT, retry_delay_seconds=LLM_RETRY_DELAY_SECONDS, chat_timeout_seconds=SENA_CHAT_TIMEOUT_SECONDS, history_max_messages=SENA_HISTORY_MAX_MESSAGES))
    raw_owner_id = os.getenv("SENA_OWNER_ID"); owner_id = parse_owner_id(raw_owner_id)
    if owner_id is None: print(f"[SENA MEMORY] owner disabled reason={'missing' if raw_owner_id is None else 'invalid'} env=SENA_OWNER_ID")
    device = detect_device()
    return AssistantManager(PersonalityManager(SENA_PERSONALITY_FILE), SessionManager(settings.chat_timeout_seconds, settings.history_max_messages), build_llm_manager(settings), settings, OwnerResolver(owner_id), MemoryManager(MemoryStore(Path("data/sena_memory.db")), MemoryPolicy(0.55, 0.70, 500), 3 if device.is_android else 5, 1600 if device.is_android else 2500))


def build_llm_manager(settings: AISettings) -> LLMManager:
    model = {"openrouter": settings.openrouter_model, "nvidia_nim": settings.nvidia_nim_model}[settings.provider_name]
    provider = create_provider(name=settings.provider_name, nvidia_base_url=settings.nvidia_nim_base_url, request_timeout_seconds=settings.request_timeout_seconds, max_tokens=settings.max_tokens, retry_count=settings.retry_count, retry_delay_seconds=settings.retry_delay_seconds)
