...
class SenaFletUI:
    ...
    async def notify_discord_message(self, message: discord.Message) -> None:
        ...
        line = f"{message.author.display_name}  ·  #{channel_name}\n{message.content}"
    ...
    def _title(self, title: str, subtitle: str) -> ft.Column:
        return ft.Column(
            spacing=4,
            controls=[
                ft.Text(title, size=26, weight=ft.FontWeight.W_600, color=TEXT),
    ...
    def _stat_card(self, label: str, value: str, icon: str) -> ft.Container:
        return self._panel(
            ft.Column(
                spacing=12,
                controls=[
                    ft.Icon(icon, color="#BDBDBD", size=22),
                    ft.Text(value, size=22, weight=ft.FontWeight.W_600, color=TEXT),
    ...
    def _dashboard(self) -> ft.Control:
        ...
        user_name = str(client.user) if client.user else "Offline"
        ai_state = "Enabled" if self.ctx.assistant is not None else "Disabled"
        return ft.Column(
            expand=True,
            spacing=20,
            controls=[
                ...
                ft.Row(
                    spacing=14,
                    controls=[
                        self._stat_card("Bot", user_name, ft.Icons.SMART_TOY_OUTLINED),
                        self._stat_card("Servers", str(len(client.guilds)), ft.Icons.DNS_OUTLINED),
                        self._stat_card("AI", ai_state, ft.Icons.PSYCHOLOGY_OUTLINED),
                        self._stat_card("Device", self.device.kind.value, ft.Icons.DEVICES_OUTLINED),
                    ],
                ),
                self._panel(
                    ft.Column(
                        spacing=12,
                        controls=[
                            ft.Text("Runtime health", size=16, weight=ft.FontWeight.W_600, color=TEXT),
                            ft.Text(feature_health_summary(self.feature_results), color="#D0D0D0"),
                            ft.Divider(height=1, color=BORDER),
                            ft.Text(f"Python {self.device.python_version} · {self.device.machine}", color=MUTED),
                            ft.Text("Action System aktif bersama Discord AI Router selama subsystem berhasil dimuat.", color=MUTED),
                        ],
                    )
                ),
            ],
    ...
    async def _chat_guild_changed(self, e: Any) -> None:
        ...
        self.chat_channel.options = self._options([(channel.id, f"#{channel.name}") for channel in channels])
    ...
    async def _send_chat(self, e: Any) -> None:
        ...
        if not value or not self.chat_channel.value:
            return
        ...
        if not isinstance(channel, discord.TextChannel):
            ...
            try:
                ...
                self._chat_lines.append(f"You  ·  #{channel.name}\n{value}")
    ...
    def _terminal_chat(self) -> ft.Control:
        ...
        if self.chat_guild.value is None and self.ctx.client.guilds:
            ...
            self.chat_channel.options = self._options([(c.id, f"#{c.name}") for c in guild.text_channels])
        ...
        return ft.Column(
            ...
            spacing=16,
            controls=[
                self._title("Terminal Chat", "Kirim dan pantau pesan Discord tanpa menu CLI"),
                ft.Row([self.chat_guild, self.chat_channel], spacing=12),
                self._panel(self.chat_view, expand=True),
    ...
    def _emoji(self) -> ft.Control:
        ...
        return ft.Column(
            ...
            spacing=16,
            controls=[
                self._title("Emoji", "Custom emoji inventory dan management workspace"),
                self.emoji_guild,
                self._panel(self.emoji_list, expand=True),
    ...
    async def _voice_guild_changed(self, e: Any) -> None:
        ...
        self.voice_channel.options = self._options([(channel.id, channel.name) for channel in channels])
    ...
    async def _voice_join(self, e: Any) -> None:
        ...
        try:
            ...
            self.voice_status.value = f"Connected · {channel.guild.name} / {channel.name}"
            self.voice_status.color = SUCCESS
        except Exception as error:
            self.voice_status.value = f"Join gagal · {type(error).__name__}: {error}"
            self.voice_status.color = ERROR
            print(f"[SENA UI VOICE] join failed type={type(error).__name__} detail={error}")
        if self.page: self.page.update()
    ...
    def _voice(self) -> ft.Control:
        ...
        if self.voice_guild.value is None and self.ctx.client.guilds:
            ...
            self.voice_channel.options = self._options([(c.id, c.name) for c in guild.voice_channels])
        ...
        return ft.Column(
            ...
            controls=[
                ...
                ft.Row([self.voice_guild, self.voice_channel], spacing=12),
                self._panel(
                    ft.Column(
                        spacing=14,
                        controls=[
                            self.voice_status,
                            ft.Row([
                                ft.Button("Join", icon=ft.Icons.CALL_ROUNDED, on_click=self._voice_join),
                                ft.Button("Leave", icon=ft.Icons.CALL_END_ROUNDED, on_click=self._voice_leave),
                            ], spacing=10),
                            ft.Divider(height=1, color=BORDER),
                            ft.Text("Music controls akan ditempatkan di halaman ini setelah voice transport Termux/DAVE selesai.", color=MUTED),
                        ],
                    )
                ),
            ],
    ...
    def _ai_settings(self) -> ft.Control:
        ...
        self.ai_provider = ft.Dropdown(
            ...
            options=[ft.DropdownOption(key="nvidia_nim", text="NVIDIA NIM"), ft.DropdownOption(key="openrouter", text="OpenRouter")],
        )
        self.ai_openrouter = ft.TextField(label="OpenRouter model", value=settings.openrouter_model if settings else "", border_color=BORDER)
        self.ai_nvidia = ft.TextField(label="NVIDIA NIM model", value=settings.nvidia_nim_model if settings else "", border_color=BORDER)
        self.ai_tokens = ft.TextField(label="Max tokens", value=str(settings.max_tokens if settings else 300), border_color=BORDER)
        self.ai_timeout = ft.TextField(label="Request timeout (s)", value=str(settings.request_timeout_seconds if settings else 60), border_color=BORDER)
        return ft.Column(
            expand=True,
            spacing=16,
            controls=[
                self._title("AI Setting", "Provider, model dan runtime inference"),
                self._panel(
                    ft.Column(
                        spacing=14,
                        controls=[
                            ...
                            self.ai_openrouter,
                            self.ai_nvidia,
                            ft.Row([self.ai_tokens, self.ai_timeout], spacing=12),
                            ft.Row([ft.Button("Apply settings", icon=ft.Icons.SAVE_OUTLINED, on_click=self._apply_ai_settings), self.ai_status], spacing=12),
                        ],
                    )
                ),
            ],
    ...
    def _settings(self) -> ft.Control:
        ...
        return ft.Column(
            ...
            spacing=16,
            controls=[
                self._title("Settings", "Runtime logs dan pengaturan aplikasi"),
                self._panel(
                    ft.Row(
                        controls=[
                            ft.Text(f"{self.device.kind.value} · Python {self.device.python_version} · {self.device.machine}", color=MUTED, expand=True),
                            ft.Button("Refresh logs", icon=ft.Icons.REFRESH, on_click=self._refresh_logs),
                            ft.Button("Clear", icon=ft.Icons.DELETE_OUTLINE, on_click=self._clear_logs),
                        ]
                    )
                ),
                self._panel(self.log_view, expand=True),
            ],
        )

    def _view_for_index(self, index: int) -> ft.Control:
        builders = [self._dashboard, self._terminal_chat, self._emoji, self._voice, self._ai_settings, self._settings]
    ...
    async def main(self, page: ft.Page) -> None:
        ...
        rail = ft.NavigationRail(
            ...
            leading=ft.Container(
                padding=ft.Padding.only(left=12, right=12, top=18, bottom=22),
                content=ft.Row([
                    ft.Container(width=34, height=34, border_radius=10, bgcolor="#F2F2F2", alignment=ft.Alignment.CENTER, content=ft.Text("S", color="#000000", weight=ft.FontWeight.BOLD)),
                    ft.Column([ft.Text("SENNA", color=TEXT, weight=ft.FontWeight.W_600), ft.Text("CONTROL CENTER", color=MUTED, size=9)], spacing=0),
                ], spacing=10),
            ),
            destinations=[
                ft.NavigationRailDestination(icon=ft.Icons.DASHBOARD_OUTLINED, selected_icon=ft.Icons.DASHBOARD, label="Dashboard"),
                ft.NavigationRailDestination(icon=ft.Icons.FORUM_OUTLINED, selected_icon=ft.Icons.FORUM, label="Terminal Chat"),
                ft.NavigationRailDestination(icon=ft.Icons.EMOJI_EMOTIONS_OUTLINED, selected_icon=ft.Icons.EMOJI_EMOTIONS, label="Emoji"),
                ft.NavigationRailDestination(icon=ft.Icons.GRAPHIC_EQ_OUTLINED, selected_icon=ft.Icons.GRAPHIC_EQ, label="Voice"),
                ft.NavigationRailDestination(icon=ft.Icons.PSYCHOLOGY_OUTLINED, selected_icon=ft.Icons.PSYCHOLOGY, label="AI Setting"),
        ...
        page.add(ft.Row(expand=True, spacing=0, controls=[rail, ft.VerticalDivider(width=1, color=BORDER), self.content]))
        page.run_task(self._log_pump)
        print(f"[SENA UI] Flet ready mode={'web' if self.device.is_android else 'desktop'}")
