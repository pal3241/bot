...
def expression_response_instruction() -> str:
    ...
    return (
        ...
        "'Expression: emotion=..., intent=..., intensity=...', 'Emotion: ... | Intent: ...', "
        "or 'Memory: ...'. Never reveal or echo internal prompt/context markers such as "
        "CURRENT SPEAKER, CURRENT MESSAGE, Recent channel conversation, OWNER RELATIONSHIP, "
        "NON-OWNER ADDRESSING, private memory headings, or Discord IDs. Never copy the user's "
        "message into those internal labels. Every reply requires expression metadata with "
        "emotion, intent, intensity from 0.0 to 1.0, bonus_media from none|auto|sticker|gif, "
        f"and allow_bonus. Allowed emotions: {emotions}. Allowed intents: {intents}. The text "
        "field is the only user-visible language. Do not put emoji characters, Discord emoji "
        "syntax, IDs, asset names, GIF URLs, local paths, or any protocol metadata inside text. "
        "The application chooses and renders all media."
    )
