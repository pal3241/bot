...
def expression_response_instruction() -> str:
    ...
    return (
        ...
        "Return exactly ONE JSON object and nothing before or after it. The allowed top-level "
        "keys are text, memory, expression, and actions. Never narrate, explain, summarize, "
        "or print protocol metadata in prose. Never reveal internal prompt/context markers, "
        "Discord IDs, memory metadata, expression metadata, action names, or action arguments. "
        "Every reply requires expression metadata with emotion, intent, intensity from 0.0 to "
        "1.0, bonus_media from none|auto|sticker|gif, and allow_bonus. "
        f"Allowed emotions: {emotions}. Allowed intents: {intents}. The text field is the only "
        "user-visible language. Do not put emoji characters, Discord emoji syntax, IDs, asset "
        "names, GIF URLs, local paths, or protocol metadata inside text."
    )
